...
function setCart(data) {
    ...
    if (storageCart === null) {
        ...
        var isExist = _.some(cart.CartLines, function (cartLine) {
            if (cartLine.Product._id === data._id) {
                cartLine.Quantity++;
                return true;
            }
        });
    ...
    var totalPrice = _.reduce(cart.CartLines, function (totalPrice, cartLine) {
        return totalPrice + parseFloat(cartLine.Product.unitPrice * cartLine.Quantity);
    }, 0);
    cart.Total = totalPrice;
    cart.Message = "Your product Chef " + data.unitPrice + " was succesfuly added to the cart";
