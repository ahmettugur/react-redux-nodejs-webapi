...
class CartList extends Component {
    removeItem(productId) {
        ...
        var item = this.props.cartLines.filter(function (el) {
            if (el.Product._id !== productId) {
                cart.Total += parseFloat(el.Product.unitPrice * el.Quantity);
                return el;
            }
        });
        cart.CartLines = item;
    ...
    renderCart() {
        return this.props.cartLines.map((cartLine) => {
            return (
                <tr key={cartLine.Product._id}>
                    <td>{cartLine.Product.productName}</td>
                    <td style={{ textAlign: 'right' }}>{this.currencyFormat(cartLine.Product.unitPrice)}</td>
                    <td style={{ textAlign: 'right' }}>{cartLine.Quantity}</td>
                    <td className="text-right">
                        <a className="btn btn-xs btn-danger pull-right" onClick={() => this.removeFromCart(cartLine.Product._id)} >
