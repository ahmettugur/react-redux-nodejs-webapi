...
export const PRODUCT_EXCEL_DOWNLOAD_URL =
  API_URL + "/api/admin/products/download";
...
export function fetchProducts(categoryId, page) {
  const request = axios.get(
    API_URL + PRODUCT_URL + "/" + categoryId + "/" + page
  );
  return {
...
export function fetchAdminProducts(page) {
  ...
  var request = axios
    .get(API_URL + PRODUCT_CRUD_URL + "/" + page, {
      headers: {
        "access-token": token.accessToken
      }
    })
...
export function productDetail(productId) {
  const request = axios.get(API_URL + PRODUCT_URL + "/" + productId);
  return {
    type: PRODUCT_DETAIL,
    payload: request
  };
